...
<body class="p-4 md:p-8 relative">
    ...
    <script>
        ...
        // State Management
        let budgetData = []; // 초기값은 빈 배열, DB에서 로드
        ...
        async function fetchData() {
            if (SCRIPT_URL === 'YOUR_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE') {
                showAlert('설정 오류', 'Google Apps Script URL이 설정되지 않았습니다. 코드에서 SCRIPT_URL을 변경해주세요.');
                renderHistory();
                return;
            }
        ...
        document.getElementById('budgetForm').addEventListener('submit', async function(e) {
            ...
            try {
                // URLSearchParams를 사용하여 form 데이터 전송 (CORS 이슈 회피를 위해 POST 대신 GET 파라미터 또는 FormData 사용, 앱스 스크립트는 FormData를 POST로 받는 것이 안정적)
                ...
                if(result.status === 'success') {
                    // 성공 시 로컬 데이터 업데이트 및 UI 갱신
                    newEntry.amount = parseInt(newEntry.amount); // 로컬에선 숫자로 저장
